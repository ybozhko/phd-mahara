...
ECONode = function (id, pid, type, dsc, w, h, c, bc, target, meta) {
    this.id = id;
    this.pid = pid;
    this.type = type;
    this.dsc = dsc;
...
ECOTree = function (obj, elm) {
    this.config = {
        ...
        nodeSelColor : "#FFFFFF",
        nodeRadius : 5,
        typeColors: ["#84AA00","#66879F"],
        levelColors : ["#1E6297","#84AA00","#AAAAFF","#CCCCFF"],
        levelBorderColors : ["#5555FF","#8888FF","#AAAAFF","#CCCCFF"],
        colorStyle : ECOTree.CS_TYPE,
        useTarget : false,
...
//CANVAS functions...
ECOTree._roundedRect = function (ctx, type, x, y, width, height, rd) {
	radius = 0;
	if (type == 0) 
		radius = 0;
	else
    ...
    for (var n = 0; n < this.nDatabaseNodes.length; n++) {
        ...
        switch (this.config.colorStyle) {
            ...
            case ECOTree.CS_LEVEL:
                ...
                break;
            case ECOTree.CS_TYPE:
            	if (node.type == ECOTree.T_CON) 
            		color = this.config.typeColors[0];
            	else
            		color = this.config.typeColors[1];
            	border = node.bc;
            	break;
        }
        ...
        if (!node._isAncestorCollapsed()) {
            ...
            switch (this.config.nodeFill) {
       		...
       		ECOTree._roundedRect(this.ctx, node.type, node.XPosition, node.YPosition, node.w, node.h, this.config.nodeRadius);
            
            this.ctx.restore();

            //HTML elements for text ... DIVs to be placed over the Canvas //word-break:break-all; draggable: true
            if (node.pid == -1) {
            	s.push('<div id="' + node.id + '" class="econode" style="position:absolute;top:' + (node.YPosition + this.canvasoffsetTop) + 'px; left:' + (node.XPosition + this.canvasoffsetLeft) + 'px; width:' + node.w + 'px; height:' + node.h + 'px;z-index:100;" ');
            }
            else {
            	s.push('<div id="' + node.id + '" draggable="true" class="econode" style="position:absolute;top:' + (node.YPosition + this.canvasoffsetTop) + 'px; left:' + (node.XPosition + this.canvasoffsetLeft) + 'px; width:' + node.w + 'px; height:' + node.h + 'px;z-index:100;" ');
            }
            ...
            //} //I needed to select only on right click
            s.push('>');
...
ECOTree.prototype.add = function (id, pid, tp, dsc, w, h, c, bc, target, meta) {
    ...
    var node = new ECONode(id, pid, type, dsc, nw, nh, color, border, tg, metadata);  //New node creation...
