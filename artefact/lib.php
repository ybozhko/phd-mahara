...
function get_fragments($artefact) {
	$examples = get_records_array('concept_example', 'aid', $artefact);
...
function get_concept_name($cid) {
	$parent = get_record('concepts', 'id', $cid);
		
	if ($parent->parent == -1) {
		return $parent->name;
	} 
	else {
		$name = get_concept_name($parent->parent);
...
function get_fragment_form_elements($data = null, $user, $artefact = null, $extension = null, $blogpost = null) {
	...
	if (in_array(strtolower($extension), $images)) {
		$elements['imageoptions'] = array(
			'type'         => 'fieldset',
			'collapsible'  => false,
			'legend'       => 'Step 1. Select image fragment',
			'elements'     => array(
				'cropbox'   => array(
					'type'  => 'html',
		...
		$elements['videooptions'] = array(
			'type'         => 'fieldset',
			'collapsible'  => false,
			'legend'       => 'Step 1. Select video fragment',
			'elements'     => array(
				'video'   => array(
					'type'  => 'html',
				...
				'start' => array(
					'type'         => 'text',
					'title'        => 'Start time',
					'description'  => 'Start time of the fragment in form HH:MM:SS',
					'defaultvalue' => $time ? sec_to_time($time[0]) : '',
				    'rules' => array(
                		'required' => true,
				...
				'end' => array(
					'type'        => 'text',
					'title'       => 'End time',
					'description' => 'End time of the fragment in form HH:MM:SS',
					'defaultvalue' => $time ? sec_to_time($time[1]) : '',
					'rules' => array(
                		'required' => true,
						'regex'    => '/([0-1][0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])/'
            		),
		...
		$elements['txtoptions'] = array(
			'type'         => 'fieldset',
			'collapsible'  => false,
			'legend'       => 'Step 1. Select file fragment',
			'elements'     => array(
				'filebox'   => array(
					'type'  => 'html',
					'name'  => 'image',
	...
	$elements['reflectionoptions'] = array(
		'type'         => 'fieldset',
		'collapsible'  => false,
		'legend'       => 'Step 2. Write reflection',
		'elements'     => array(
			'title' => array(
				'type'         => 'text',
				'title'        => 'Title',
	...
	$elements['conceptoptions'] = array(
		'type'         => 'fieldset',
		'collapsible'  => false,
		'legend'       => 'Step 3. Select concept',
		'elements'     => array(
			'concept' => array(
				'type'         => 'select',
				'title'        => 'Concept',
