...
if ($extradata->view) {
...
else if ($extradata->map) {
	if(!can_view_map($extradata->map)) {
		json_reply('local', get_string('noaccesstoview', 'view'));		
	}
	$view = null;
	$artefact = null;
	...
	$limit    = param_integer('limit', 10);
	$offset   = param_integer('offset');
	$map = new ConceptMap($extradata->map);
	
	$data = ArtefactTypeComment::get_comments($limit, $offset, null, $view, $artefact, $map);
}
