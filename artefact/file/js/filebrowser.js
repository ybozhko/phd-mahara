...
function FileBrowser(idprefix, folderid, config, globalconfig) {
    ...
    this.fragments_init = function (id) { window.location.href = self.config.wwwroot + 'artefact/file/fragments.php?item=' + id; }
        ...
        if (self.config.edit || self.config.editmeta) {
            forEach(getElementsByTagAndClassName('input', null, 'filelist'), function (elem) {
                ...
                if (name && name[1]) {
                    ...
                    else if (name[1] == 'fragments') {
                    	var id = name[2];
                    	connect(elem, 'onclick', function (e) {
                            e.stop();
                            self.fragments_init(id);
                            return false;
                        });
                    }
                }
