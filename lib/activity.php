...
function activity_get_mapaccess_users($map, $owner, $type) {
    $type = activity_locate_typerecord($type);
    $sql = "SELECT userid, u.*, p.method, ap.value AS lang
                FROM (
                SELECT (CASE WHEN usr1 = ? THEN usr2 ELSE usr1 END) AS userid 
                    FROM {usr_friend} f
                    JOIN {concept_maps} v ON (v.owner = f.usr1 OR v.owner = f.usr2)
                    JOIN {concept_access} vu ON vu.map = v.id
                        WHERE (usr1 = ? OR usr2 = ?) AND vu.accesstype = 'friends' AND v.id = ? 
                UNION SELECT usr AS userid 
                    FROM {concept_access} u 
                        WHERE u.map = ?
                UNION SELECT m.member 
                    FROM {group_member} m
                    JOIN {concept_access} vg ON vg.group = m.group
                    JOIN {group} g ON (g.id = vg.group AND g.deleted = 0 AND g.viewnotify = 1)
                    JOIN {group_member} og ON (g.id = og.group AND og.member = ?)
                        WHERE vg.map = ? AND (vg.role IS NULL OR vg.role = m.role)
                ) AS userlist
                JOIN {usr} u ON u.id = userlist.userid
                LEFT JOIN {usr_activity_preference} p ON p.usr = u.id AND p.activity = ?
                LEFT JOIN {usr_account_preference} ap ON ap.usr = u.id AND ap.field = 'lang'";
    $values = array($owner, $owner, $owner, $map, $map, $owner, $map, $type->id);
    if (!$u = get_records_sql_assoc($sql, $values)) {
        $u = array();
    }
    return $u;
...
class ActivityTypeMapaccess extends ActivityType { 
    protected $map;
    protected $owner;
    protected $oldusers; // this can be empty though
    ...
    public function __construct($data, $cron=false) { 
        parent::__construct($data, $cron);
        if (!$mapinfo = get_record_sql('
            SELECT v.name, v.owner, u.id, u.username, u.preferredname, u.firstname, u.lastname, u.staff, u.admin,
            FROM {concept_maps} v
                LEFT JOIN {usr} u ON v.owner = u.id
            WHERE v.id = ?', array($this->map))) {
            if (!empty($this->cron)) { // probably deleted already
                return;
            }
            throw new MapNotFoundException(get_string('mapnotfound', 'error', $this->map));
        }
        $this->url = get_config('wwwroot') . 'concept/viewmap.php?id=' . $this->map;
        $this->users = array_diff_key(
            activity_get_mapaccess_users($this->map, $this->owner, $this->get_id()),
            $this->oldusers
        );
        $this->name = $mapinfo->name;
        if ($this->users) {
   			if ($mapinfo->owner) {
                $this->ownername = display_name($mapinfo, null, true);
            }
        }
        $this->add_urltext(array('key' => 'Map', 'section' => 'concept'));
    }

    public function get_subject($user) {
        return get_string('newmapaccesssubject', 'activity');
    }
    
    public function get_message($user) {
        if ($this->ownername) {
            return get_string_from_language($user->lang, 'newmapaccessmessage', 'activity',
                                            $this->name, $this->ownername);
        }
        return get_string_from_language($user->lang, 'newmapaccessmessagenoowner', 'activity', $this->name);
    }
    
    public function get_required_parameters() {
        return array('map', 'owner', 'oldusers');
    }	
}
