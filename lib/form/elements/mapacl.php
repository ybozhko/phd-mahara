...
/**
 * Provides an element to manage a map ACL
 *
 * @param array    $element The element to render
 * @param Pieform  $form    The form to render the element for
 * @return string           The HTML for the element
 */
function pieform_element_mapacl(Pieform $form, $element) {
    $smarty = smarty_core();
        ...
        foreach ($value as $key => &$item) {
            if (is_array($item)) {
                if (in_array($item['type'], $presets)) {
                    ...
                    $item['name'] = pieform_render_mapacl_getvaluebytype($item['type'], $item['id']);
                }
    ...
    $smarty->assign('mapid', $form->get_property('mapid'));
    $smarty->assign('formname', $form->get_property('name'));
    return $smarty->fetch('form/mapacl.tpl');
}

function pieform_render_mapacl_getvaluebytype($type, $id) {
    switch ($type) {
        case 'user':
...
function pieform_element_mapacl_get_value(Pieform $form, $element) {
    global $USER;
    $values = null;
