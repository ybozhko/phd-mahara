...
if (!$USER->can_edit_map($map)) throw new AccessDeniedException();

$data = get_records_array('concept_access_log', 'map', $mapid);

foreach ($data as $item) {
	if ($item->usr) {
		$user = get_user($item->usr);
		$item->token = $user->firstname . ' ' . $user->lastname;
	}
	else if ($item->group) {
		$group = get_record('group', 'id', $item->group);
		$item->token = $group->name;
	}
	else if ($item->role) {
		//Role access: don't know what it is
	}
}

$smarty = smarty(array('tablerenderer'));
$smarty->assign('data', $data);
$smarty->assign('map', $mapid);
