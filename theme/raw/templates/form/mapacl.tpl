...
function renderAccessListItem(item) {
    ...
    var allowmfdbk = INPUT({
                        'type': 'checkbox',
                        'name': 'accesslist[' + count + '][allowmapcomments]',
                        'id'  :  'allowmapcomments' + count,
                        'value':  1});
    var allowefdbk = INPUT({
        				'type': 'checkbox',
        				'name': 'accesslist[' + count + '][allowexamplecomments]',
        				'id'  :  'allowexamplecomments' + count,
        				'value':  1});
    ...
    var dateInfo = TABLE(null,
        TBODY(null,
            ...
            TR({'class':'comments' + (allowcomments ? ' hidden' : '')},
                TH(null, 'Allow comments' + ':'),
                TD(null, allowmfdbk, 'Map', allowefdbk, 'Examples', approvefdbk, 'Moderate')
            )
        )
    );

    if (item['allowmapcomments']==1) {
        setNodeAttribute(allowmfdbk,'checked',true);
    }
    if (item['allowexamplecomments']==1) {
        setNodeAttribute(allowefdbk,'checked',true);
    }
    if(item['allowexamplecomments']==1 || item['allowmapcomments']==1) {
    ...
    connect(allowmfdbk, 'onclick', function() {
        if (allowmfdbk.checked || allowefdbk.checked) {
    ...
    connect(allowefdbk, 'onclick', function() {
        if (allowefdbk.checked || allowmfdbk.checked) {
            approvefdbk.disabled = false;
        }
        else {
            approvefdbk.disabled = true;
    ...
    if (item.type == 'email') {
    	item.name = 'E-mail: ' + item.id;
    } 

    if (item.type == 'token') {
        item.name = config.wwwroot + 'concept/viewmap.php?t=' + item.id;
    } 
    var name = item.name;
    if (item.type == 'user') {
